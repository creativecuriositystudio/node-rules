    ...
    var updateFact = function (facts, key, operations) {
        ...
        _.each(_.isArray(operations) ? operations : [operations], function (subOps, i) {
            _.each(subOps, function (uncompiledValue, opKey) {
                ...
                switch (opKey) {
                case '$permute':
                  // Generate permutations of fact based on arrays referenced in value
                  var perms = _.reduce(_.castArray(value), (perms: any, path: string) => {
                      return _.chain(dig(fact, path))
                        .castArray()
                        .map(obj => {
                          var newPerm = _.cloneDeep(fact);
                          bury(newPerm, path, obj);
                          return perms ?
                            perms.forEach(perm => _.merge(perm, newPerm)) :
                            newPerm;
                        })
                        .flatten()
                        .value();
                    ...
                    undefined);
                  return bury(facts, key, perms || [fact]);
