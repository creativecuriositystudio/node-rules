(function() {
    ...
    var testFact = function (facts, key, operations) {
        ...
        return _[isAnd ? 'all' : 'any'](operations.$or || operations, function (uncompiledValue, key) {
            ...
            if (!isValue) throw 'invalid condition value for: ' + key;
            ...
            case '$contains': return fact.includes(value);
            case '$notContains': return !fact.includes(value);
            case '$match': return !!fact.match(value);
            case '$notMatch': return !fact.match(value);
            default: throw 'invalid condition operator: ' + key;
            }
        });
    };
    ...
    var updateFact = function (facts, key, operations) {
        ...
        _.each(_.isArray(operations) ? operations : [operations], function (subOps, i) {
            _.each(subOps, function (uncompiledValue, opKey) {
                var [isValue, value] = compileValue(facts, uncompiledValue);
                if (!isValue) throw 'invalid consequence value for: ' + key;
                ...
                throw 'invalid consequence operator: ' + opKey;
            });
        });
    };
            ...
            var API = {
                ...
                "when": function(outcome) {
                    if (outcome) {
                        ...
                        var ruleRef = _rules[x].id || _rules[x].name || 'index_'+x;
