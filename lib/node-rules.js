(function() {
    ...
    var cloneDeep = function (val) {
        return _.isObject(val) ? _.mapObject(val, cloneDeep) :
          _.isArray(val) ? val.map(function (val) { return cloneDeep(val); }) :
          _.isDate(val) ? new Date(thing) :
          _.isRegExp(val) ? new RegExp(thing) :
          val === undefined || val === null ? val :
          val.__proto__.constructor(val);
    }

    var pathParts = function (prop, root) {
        return _.isString(prop) ? (prop === '.' ? root : prop[0] === '.' ? root + part : prop).split(/\.|]/) :
            _.isArray(prop) ? (root || []).concat(prop) :
            [prop];
    }

    var dig = function (obj, prop, root) {
        return _.reduce(
            pathParts(prop, root),
            function (memo, prop) {
    ...
    var bury = function (obj, prop, value, root) {
        return _.reduce(
            pathParts(prop, root),
            function (memo, prop, i, parts) {
                var fullProp = typeof prop === 'string' && prop[0] === '[' ? +prop.slice(1) : prop;
    ...
    var compileValue = function (facts, value, rootPath) {
        if (!_.isObject(value)) return [true, value];

        switch (Object.keys(value)[0]) {
        case '$field': return [true, dig(facts, value.$field, rootPath)];
        case '$min': return _.min(compileValue(facts, value.$min, rootPath));
        case '$max': return _.max(compileValue(facts, value.$max, rootPath));
        }

        return [false];
    };

    var checkConditions = function (facts, conds, isAnd, rootPath) {
        if (_.isArray(conds)) {
            return _[isAnd ? 'all' : 'any'](conds, function (subConds, key) {
                return checkConditions(facts, subConds, true, rootPath);
            });
        }
        ...
        return _[isAnd ? 'all' : 'any'](_.pairs(conds), function (pair, i) {
            ...
            switch (key) {
            case '$and': return checkConditions(facts, subConds, true, rootPath);
            case '$or': return checkConditions(facts, subConds, false, rootPath);
            case '$not': return !checkConditions(facts, subConds, isAnd, rootPath);
            }

            var [isValue, value] = compileValue(facts, subConds, rootPath);
            ...
            var fact = dig(facts, key, rootPath);

            if (value !== undefined) return fact == value;

            return checkConditions(fact, subConds, true);
        });
    };
            ...
            (function FnRuleLoop(x) {
                var API = {
                    ...
                    "when": function(outcome) {
                        if (outcome) {
                            var _consequence = rules[x].consequence || rules[x].consequences;
                ...
                if (x < rules.length && complete === false) {
                    var _condition = rules[x].condition || rules[x].conditions;
                    var ruleRef = getRuleRef(rules[x], x);
    ...
    RuleEngine.prototype.execute = function (fact, forEach) {
        ...
        if (forEach) {
            ...
            var facts = _.reduce(
                ...
                function (perms, path) {
                    return _.chain(perms)
                        .map(function (perm) {
                            var objs = dig(perm, path);
                            return (_.isArray(objs) ? objs : [objs]).map(function (obj) {
            ...
            return Promise.all(facts.map(function (fact) { return doExecute(cloneDeep(fact), rules, ignoreFactChanges); }));
        }
