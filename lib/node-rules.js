(function() {
    ...
    var dig = function (obj, prop, root) {
        return _.reduce(
            typeof prop === 'string' ? ((prop[0] === '.' ? root : '') + prop).split(/\.|]/) :
    ...
    var compileValue = function (facts, value, rootPath) {
        ...
        default: throw new Error('Invalid value operator: ' + key);
        }
        ...
        _.each(_.isArray(operations) ? operations : [operations], function (subOps, i) {
            if (subOps.$each) {
                return _.each(fact, function (subFact, subKey) {
                    if (!subOps.$where || checkConditions(facts, subOps.$where, true, key))
